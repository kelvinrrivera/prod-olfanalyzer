# How to use Dockerized Olfanalyzer

Use the Dockerized version of OLFANALYZER for a much faster and complete startup of OLFANALYZER.

### Minimum Requirements

> [!TIP]
> Running OLFANALYZER on AWS/GCP/Azure?
...
\*OLFANALYZER by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*OLFANALYZER by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized OLFANALYZER!
...
> the docker container using OLFANALYZER as `localhost:xxxx` will not resolve for the host system.
>
...
```shell
docker pull mintplexlabs/olfanalyzer
```

<table>
<tr>
<th colspan="2">Mount the storage locally and run OLFANALYZER in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
```shell
export STORAGE_LOCATION=$HOME/olfanalyzer && \
...
$env:STORAGE_LOCATION="$HOME\Documents\olfanalyzer"; `
...
services:
  olfanalyzer:
    image: mintplexlabs/olfanalyzer
    ...
    volumes:
      - olfanalyzer_storage:/app/server/storage
    restart: always
...
Go to `http://localhost:3001` and you are now using OLFANALYZER! All your data and progress will persist between
...
- `git clone` this repo and `cd olfanalyzer` to get to the root directory.
- `touch server/storage/olfanalyzer.db` to create empty SQLite DB file.
- `cd docker/`
...
The integrations below are templates or tooling built by the community to make running the docker experience of OLFANALYZER easier.

### Use the Midori AI Subsystem to Manage OLFANALYZER

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the OLFANALYZER docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in OLFANALYZER.
